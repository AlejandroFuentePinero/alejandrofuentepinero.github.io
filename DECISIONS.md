...
164. **The grant and award years render again.** Kramdown read the leading "2022." on each record line as a nested ordered list, so every entry rendered as "1." with its year swallowed. The periods are now backslash-escaped (`2022\.`), a source-only markdown escape: the rendered text is byte-identical to the record as written, so the never-rewrite rule holds. *Superseded the same day by the honor cards (DECISIONS 165).*

165. **Grants and awards become honor cards.** Owner direction: boxes over bullets, styled to the system. Component 16, the honor card, is a two-column grid of hairline tiles (mono meta row with years left and amount right, funder or award name in the body face, detail muted, no hover since nothing links), shown on the styleguide. The entries move out of work.md record markers into `_data/grants.yml` and `_data/awards.yml`, rendered by `grants-awards.html`; the data files carry the record warning instead. Two owner-directed merges: the 2021 Skyrail grant and its 2022 top-up read as one $10,000 entry, and the four Salamanca undergraduate scholarships read as one entry, 2010 to 2013, €24,000 at €6,000 a year. Both totals are sums of the recorded per-year figures, no new numbers. The #grants-awards, #grants and #awards anchors survive, so the three redirect stubs still land.

166. **The /work/ disclosures retire: open sections, timeline and cards.** Owner direction. Volunteering and professional development merge into one reverse-chronological section rendered as a dense timeline variant (`timeline--dense`: no summary, body-face titles, tighter rows, shown on the styleguide beside component 12), and media engagement becomes honor cards with the kind and outlet in the meta row and the linked headline as the title. Both records move to `_data/volunteering.yml` and `_data/media.yml`, following the grants pattern, and the JCU vertebrate monitoring entry closes at 2025 on the owner's word. The disclosure component itself survives on /skills/ and the paper pages. The heading-spacing rule extends to timelines and card grids.

167. **The volunteering timeline goes back behind a disclosure.** Owner direction: twenty rows made the open section too long for /work/. The h2 stays visible so the #volunteering anchor and the section rhythm hold, and the dense timeline collapses beneath it in the existing disclosure component with the summary "Show the full timeline, 2013 to 2025". Media and grants stay open. Partially walks back the open-sections half of DECISIONS 166; the merged single section, the dense timeline and the data files all stand.

168. **The /skills/ skills section condenses to three capability areas.** Owner direction, scoped to the skills section only: certificates and education stay untouched. The Stack disclosure and the four depth disclosures merge into three areas (AI engineering, statistical modelling under uncertainty, data and software foundations), each a one-line claim plus five bullets with tools named inline, no standalone tool list. The field and modelling disclosure leaves the page: its methods are covered by the statistical area, which now closes with a line pointing to /research/, and its include file is deleted with the record text preserved in git history (supersedes the one-consumer half of DECISIONS 160). The languages line is rewritten fresh outside record markers at the foot of the section, and IELTS joins the acronym allowlist as a proper name.

169. **Capability cards and anchor chips ship as component 17.** The three /skills/ areas render as bordered single-column tiles (title, muted one-line claim, at most five bullets), styled after the honor card with no hover since nothing links, shown on the styleguide. The skills row chips become anchors: `_data/skills.yml` entries gain an `anchor` field pointing at a capability card, `skills-row.html` renders `a.chip` with the underline dropped and the border carrying the hover signal, so every chip navigates to its proof. The row's only consumers are /skills/ and the styleguide, so no other page changes.

170. **The degrees become honor cards.** Owner direction: boxes like the other pages, Cum laude as an accent label, the description inside the box. The degree record moves from the education-degrees.md record markers into `_data/education.yml`, which carries the record warning, following the grants pattern (DECISIONS 165): degree titles, institutions, years and the honour split into fields byte for byte. Tiles render newest first (Ph.D., M.S., B.S.), matching the honor-card convention, with the year in the meta row, the honour flag right-aligned in `--accent-deep`, the degree in the body face, then the institution and the description line muted. The three description lines leave the checked page prose and are held to Tier B by hand, like the grant details. No new colour: the flag uses the existing accent tokens.

168. **The coverage line leaves the /work/ media cards.** Owner direction: the "Total coverage: 70 national stories" detail comes off the JCU possum card, so every media card now reads kind, outlet, headline and nothing else. The Mediaportal report link survives untouched on the ringtail possum paper page, where the coverage claim keeps its source.

170. **Link audit: the twin links once, dead journal links fall back to the DOI.** The digital twin pointed at the same app three ways (on-site chat, raw hf.space URL, Hugging Face Space page). The project page and the /apps/ card now carry only the on-site chat at /digital-twin/ and the GitHub source, and the stale "also lives on the home page" note comes off the card since the home embed retired. A full crawl of the built site found no broken internal links or anchors. One external failure: biotaxa.org is down platform-wide, so the Siri et al. 2025 paperurl and the forest-gap project's journal link now use the paper's DOI (https://doi.org/10.37828/em.2025.88.11), the durable identifier that will follow the journal when it returns. The citation text is untouched and the local PDF still serves the record.

171. **Certificates open up as capability cards.** Owner direction: the accordion goes, the record renders in the open like the depth-by-area list. The disclosure in `education-certificates.html` becomes a `capability-list` of seven cards, one per training area, under a visible "Certificates and training" h3 (anchor #certificates). Every course title, link and description is byte-identical inside the record markers; the only editorial move is card order, which now leads with AI engineering and Python before the research-era material, matching the site's positioning. The defensive page lead comes off /skills/ (owner direction), so the meta description re-anchors to the section's opening prose per the verbatim-substring rule. Two spacing rules join the system: `.chip-row + p` gets `margin-top: var(--space-lg)` so the chip row clears the prose below it, and `.honor-grid + h3` joins the heading-air rule from DECISIONS 162.

172. **The editorial service list opens up as journal tiles.** Owner direction: boxes instead of the bullet list, no accordion, small boxes in three columns since each holds only a journal name. The disclosure on /research/ becomes component 18, the journal tile: a `journal-grid` of hairline boxes, three columns from 56.25em and two below, centred name in the small step, no hover since nothing links, shown on the styleguide. A visible "Editorial service" h3 replaces the summary, with no lead line since the page sits at its 700-word budget. The eight journal names are unchanged.

172. **Two distillation gaps close: MCP and cloud.** A diff of the condensed skills section against the old /datascience/skills/ page found two certified claims with no trace in the new body: Model Context Protocol work (FastMCP servers and clients, remote deployment) and AWS at practitioner level. Each returns as a sixth bullet on its card (AI engineering and foundations), loosening the five-bullet line in DECISIONS 169 to six where a certified claim would otherwise vanish. The other cuts stand: deep learning fundamentals, A/B testing, multimodal inputs, recommenders and the visualisation library list were the page's weakest claims and stay out.

173. **The editorial service list becomes journal tiles.** Owner-built: the /research/ disclosure opens into component 18, a grid of small bordered boxes, one journal name per tile, three columns wide and two on small screens, centred text, no hover since the tiles do not link. Shown on the styleguide as a post-launch component. The eight journal names are unchanged.

174. **The nomination link gets its own section.** Owner direction: the threatened species link is important and was not obvious as a trailing sentence under Publications. A new "Threatened species nominations" section (anchor #threatened-species) sits between Publications and Talks, holding the existing sentence and link unchanged.

175. **The post retires and the chapters become chapter rows.** Owner direction: the post repeated the chapter list, and the flat list gives way to rows. The one blog post is deleted, with the publisher's description record preserved in git history, and both its URLs (`/posts/action-plan-australian-birds-2021/` and the Phase 2 `/posts/2012/08/blog-post-1/` redirect) land on `/research/#book-chapters` via stubs, logged in MIGRATIONS.md. The sitemap's Posts section goes with it. Component 19, the chapter row, renders each account as a talk row without the year column: the account title splits out of the citation at render time in Liquid and carries `--accent-deep` (owner asked for orange, the deep accent keeps contrast), the author list stays muted with "de la Fuente A." bold, and the reassembled text is byte-identical to the record, which never leaves `_data/book_chapters.yml`. Shown on the styleguide.

176. **The course list opens up as a talks-style timeline.** Owner direction: no accordion. The record moves from the `james_cook_university.md` markers into `_data/courses.yml` (record warning, fields split byte for byte: year, course, level, role, and the guest lecture title), following the grants pattern. The teaching entry renders the rows with the existing talk-row classes, so no new component. The per-entry venue line comes off the /research/ teaching loop since the intro paragraph and the rows carry the venue themselves.

177. **Mentoring reduces to the student row.** Owner direction: just the student name, no project abstract. The entry becomes one talk row: 2025, Olivia Bond as the title, the venue line, and the project title record as the excerpt with its text unchanged. The result paragraph and the abstract disclosure leave the page, the abstract record preserved in git history like DECISIONS 168.

178. **Mentoring promotes to its own section.** Owner direction: mentoring sat inside Teaching as an h3, one level below the sections around it. It moves into its own h2 section (anchor #mentoring) after Teaching, the student row's heading rises from h4 to h3 to keep the outline gapless, the teaching loop filters the entry out, and the `/teaching/mentoring/` redirect re-points from `#teaching` to `#mentoring` with the MIGRATIONS.md line updated. Row markup and record text are unchanged.

179. **Project cards move to a finding-plus-proof template at 25 words.** Owner-approved rewrite of 13 excerpts on /projects/: sentence 1 states the plain-language finding or capability, sentence 2 the proof, jargon glossed (deviance, assemblages, altitudinal migration, escalator to extinction), result numbers ahead of input-scale numbers. The task brief allowed 30 words per card, but `prose_check.py` enforces 25, and the checker wins over the brief, so every card was tightened to 25 or fewer. Three cards already met the template and stand unchanged (LLM Engineering Lab, climatic drivers, rainforest bird declines). The ringtail card was restructured despite the brief's light-touch note because it led with method, against the result-first rule. Open item: the digital twin page never states the judge scale ceiling, so its card says "scores 4.56" without "of 5" until the scale is confirmed.

180. **The CV PDF freezes into a pointer.** Owner call on FINDINGS 26: replace the unlinked, stale CV with a redirect to /work/. GitHub Pages cannot HTTP-redirect a `.pdf` path (redirect pages are HTML and the extension fixes the content type), so the closest faithful mechanism is a frozen one-page PDF at the same URL stating the CV is retired and linking to /work/. Old external links keep resolving to a valid PDF, one click from the live page. `scripts/generate_cv_pdf.py` is deleted, MAINTENANCE.md drops the regeneration procedure and the pre-commit ritual's /work/ trigger, and MIGRATIONS.md records the content change.

181. **Closeout record repairs, owner-approved.** The FINDINGS owner calls were put to the owner on 2026-08-08 and these edits to verbatim records were approved: "Graig, M." becomes "Craig, M." across the five nomination author lists and the one "Freeman AND." without a comma matches its siblings (FINDINGS 6), the three doubled spaces inside talk locations collapse (FINDINGS 16), and "Advanced statistics" takes Title Case like its course-list siblings (FINDINGS 18). FINDINGS 8 needed no edit: the award renders from `_data/awards.yml` as "Best talk", worded from the owner's CV, so the long recorded title no longer appears on any surface.

182. **Two citations move to their versions of record, and two binomials are corrected.** Owner-approved under FINDINGS 14: the possum paper citation now reads Diversity and Distributions, 29(1), 173-183 with year 2023 (the issue year of the version of record; the page's date field keeps the 2022 online-first date, and the permalink is a slug, not a claim), and the 2023 Global Change Biology citation reads 29(8), 2132-2140, both verified against the publisher and PubMed. Under FINDINGS 19 the chapter citations correct "Oreoscupus" to Oreoscopus and "Heteremyias" to Heteromyias (the styleguide's sample chapter row follows). The duplicated "pp. 744-746" on items 1 and 13 stays: the true range is only verifiable against the printed book, which the owner will check.

183. **The 7PH figures refresh and the Job Intelligence Engine card gets a wake note.** Owner-approved: the 7PH project page pilot count moves from 1,083 to 1,086, read from the live 7phgraph.com footer on 2026-08-08, with the other three figures already in agreement (FINDINGS 9). The Job Intelligence Engine card's empty note becomes "Free hosting, slow to wake. Reload if the loading page stalls.", matching the birds card's phrasing, honest about the rough wake, and short enough to hold /apps/ inside its 200-word budget (FINDINGS 24).

184. **Four owner calls settle as no-change.** On 2026-08-08 the owner declined: the El Austral line stays as an honest description of unverifiable print coverage (FINDINGS 13), the threatened-species nomination list keeps its record markers and the h1-to-h3 skip (FINDINGS 20), `files/app_demo.gif` stays a gif (FINDINGS 21), and the broken Ecologica Montenegrina DOI goes unreported since the site already links the DOI as the durable identifier (FINDINGS 23). With FINDINGS 26 settled by DECISIONS 180, every open owner call from the Gate 7 closeout list is now resolved.

185. **The refurbishment closes and the refurb branches retire.** The owner completed the 12-step manual browser pass on 2026-08-08 and directed the retirement, waiving the remainder of the soak week. At retirement main, refurb/main and both origin refs sat level at 4c594c2 with no open PRs and no unmerged branches, so the 14 phase branches and refurb/main are deleted locally and on origin with nothing lost. From here MAINTENANCE.md governs all work: branch from main, PR into main.

186. **The projects stat relabels to "Open-source projects".** Owner direction: the band's "Projects" count read as a total, and most of the owner's work is private, so the label now scopes the number to what is public. The count stays derived from the `projects` collection and is unchanged at 16. Raised with the owner and accepted: 7 of the 16 link a public repository, the other 9 are write-ups of published papers, so "open-source" is broader here than "has a GitHub link". No template digit is introduced and the new label matches "Peer-reviewed papers" in length, so the band layout is untouched.
